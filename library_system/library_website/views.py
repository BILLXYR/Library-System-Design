...
mydb = mysql.connector.connect(
    ...
    passwd="xiaokeai" # change to your password
)
...
def calculateFine():
    ...
    totalUnPaidfine = myresult10[0][0]

    mycursor14 = mydb.cursor()
    sql17 = "SELECT lastUpdatedDate FROM fine WHERE memberID = %s"
    mycursor14.execute(sql17, (str(userLoginName),))
    myresult17 = mycursor14.fetchall()
    lastDate = myresult17[0][0]
    sql18 = "Update Fine SET lastUpdatedDate = %s where memberID = %s"
    mycursor14.execute(sql18, (curDate, str(userLoginName), ))
    mydb.commit()
    ...
    if len(myresult1) != 0:
        ...
        for i in myresult1:
            ...
            if dueDate < curDate:
                dueDateMinusLastUpdate = (dueDate - lastDate).days
                print(dueDateMinusLastUpdate)
                if (dueDateMinusLastUpdate >= 0):
                    print("in here")
                    daysPassed = (curDate - dueDate).days
                    fine = daysPassed * ratePerDay
                else:
                    print("in there")
                    daysPassed = (curDate - lastDate).days
                    fine = daysPassed * ratePerDay
                    print(daysPassed)
                print(fine)
...
def adminAutoCancellation():
    ...
    for details in myresult2:
        ...
        if (numbersOfDaysPassed) > 1:
            ...
            mydb.commit()
    tempAdminUserName = userLoginName
    sql10 = "SELECT memberID FROM User"
    mycursor11.execute(sql10)
    myresult10 = mycursor11.fetchall()

    for allUsers in myresult10:
        userLoginName = allUsers[0]
        calculateFine()
    userLoginName = tempAdminUserName
